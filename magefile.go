...
func Build() {
	banner := figure.NewFigure("KornFlake", "", true)
	banner.Print()
...
func (Bin) Kornflake() error {
	return goBuild("go.zenithar.org/kornflake/cli/kornflake", "kornflake")
}
...
func goBuild(packageName, out string) error {
	...
	varsSetByLinker := map[string]string{
		"go.zenithar.org/kornflake/internal/version.Version":   tag(),
		"go.zenithar.org/kornflake/internal/version.Revision":  hash(),
		"go.zenithar.org/kornflake/internal/version.Branch":    branch(),
		"go.zenithar.org/kornflake/internal/version.BuildUser": os.Getenv("USER"),
